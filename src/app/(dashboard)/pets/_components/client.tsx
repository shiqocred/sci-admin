...
import { useDeletePet, useGetPets } from "../_api";
...
export const Client = () => {
  const [{ dialog, petId, sort, order }, setQuery] = useQueryStates(
    {
      dialog: parseAsString.withDefault(""),
      petId: parseAsString.withDefault(""),
  ...
  const { page, metaPage, limit, setLimit, setPage, setPagination } =
    usePagination();
  const { data, refetch, isRefetching, isSuccess, isPending } = useGetPets({
    q: searchValue,
    p: page,
    limit,
    sort,
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      ...
      <CreateEditDialog
        ...
        onOpenChange={() => {
          setQuery({ petId: null, dialog: null });
        }}
        petId={petId}
      />
